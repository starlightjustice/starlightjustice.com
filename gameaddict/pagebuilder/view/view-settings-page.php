...
switch($action) {
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$templates = $this->get_templates();
			$selected_template_object = get_post($selected_template_id);
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'addict') . '<strong>' . $template_name . '</strong>' . __(' page template has been successfully created', 'addict') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$errors .= '</ul>';
			$messages[] = '<div id="message" class="error"><p>' . __('Sorry, the operation was unsuccessful for the following reason(s): ', 'addict') . '</p>' . $errors . '</div>';
		}
		break;
	case 'update' :
		$blocks = isset($_REQUEST['blocks']) ? $_REQUEST['blocks'] : '';
		$this->update_template($selected_template_id, $blocks, $template_name);
		//refresh templates var
		$templates = $this->get_templates();
		$selected_template_object = get_post($selected_template_id);
		$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'addict') . '<strong>' . $template_name . '</strong>' . __(' page template has been updated', 'addict') . '</p></div>';
		break;
	case 'delete' :
		...
		$selected_template_object = get_post($selected_template_id);
		$messages[] = '<div id="message" class="updated"><p>' . __('The template has been successfully deleted', 'addict') . '</p></div>';
		break;
}
global $current_user;
	...
	<div id="page-builder-frame">
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e('Available Blocks', 'addict') ?></span><span id="removing-block"><?php _e('Deleting', 'addict') ?></span></h3>
				<div class="inside">
		...
		<div id="page-builder-fixed">
			<div id="page-builder">
				<div class="pb-tabs-nav">
					<div class="pb-tabs-arrow pb-tabs-arrow-left">
						<a>&laquo;</a>
					</div>
						...
						<div class="pb-tabs">
							...
							foreach ( (array) $templates as $template ) {
								if($selected_template_id == $template->ID) {
									echo '<span class="pb-tab pb-tab-active pb-tab-sortable">'. htmlspecialchars($template->post_title) .'</span>';
								} else {
									echo '<a class="pb-tab pb-tab-sortable" data-template_id="'.$template->ID.'" href="' . esc_url(add_query_arg(
										array(
							...
							<span class="pb-tab pb-tab-add pb-tab-active"><abbr title="Add Template">+</abbr></span>
							<?php } else { ?>
							<a class="pb-tab pb-tab-add" href="<?php
					...
					<div class="pb-tabs-arrow pb-tabs-arrow-right">
						<a>&raquo;</a>
					</div>
				</div>
				<div class="pb-wrap pbdiv">
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
						<div id="pb-header">
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										<label class="open-label" for="template-name">
											<span><?php _e('Template Name', 'addict') ?></span>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'addict' ) : __( 'Save Template', 'addict' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
								...
								<input type="hidden" id="pb-nonce" name="pb-nonce" value="<?php echo wp_create_nonce('pb-settings-page-nonce') ?>"/>
						</div>
						<div id="pb-body">
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									<br/><br/>
									When you have finished building your custom page template, make sure you click the Save Template button.', 'addict');
									echo '</p>';
								} else {
						...
						<div id="pb-footer">
							<div class="major-publishing-actions cf">
								<div class="publishing-action">
									<?php if(!empty($selected_template_id)) {
										submit_button( __( 'Save Template','addict' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) );
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
						</div>
