...
add_action('wp_enqueue_scripts', 'WPWall_ScriptsAction');
...
function WPWall_ScriptsAction()
{
	if (!is_admin())
	{
        ...
        wp_enqueue_script('wp_wall_script',  get_template_directory_uri() . '/js/wp-wall.js', array('jquery', 'jquery-form'), '',true);
	...
	if ( $comments ) {
		...
		foreach ($comments as $comment) {
			...
			if ($author_data)
				...
				$highlight="";
			$icon = (1 == $comment->user_id) ? '<i class="icon-star"></i> ' : '<i class="icon-user"></i> ';
...
function WPWall_GetOptions()
{
 ...
 $options = array(
	 'wall_title' => __('Wall', 'addict'),
	 'wall_reply' => __('Leave a reply', 'addict'),
	 'wall_comments' => 5,
	 'wall_wrap' => 25,
	 'disable_new' => '',
	 'only_registered' => '',
